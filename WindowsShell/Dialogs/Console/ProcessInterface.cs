    ...
    public class ProcessInterface
    {
        ...
        void outputWorker_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            ...
            if (e.UserState is string)
            {
                ...
                FireProcessOutputEvent((e.UserState as string).Replace("\r\r", "\r").Replace("\r\r", "\r"));
            }
        }
            ...
            while (outputWorker.CancellationPending == false)
            {
                ...
                do
                {
                    if (outputReader != null)
                    {
                        ...
                        string sr = Encoding.GetEncoding(866).GetString(Encoding.Default.GetBytes(builder.ToString()));
                        outputWorker.ReportProgress(0, sr);                       
                    }                    
                } while (count > 0);
            ...
            while (errorWorker.CancellationPending == false)
            {
                ...
                do
                {
                    if (errorReader != null)
                    {
                        ...
                        builder.Append(buffer, 0, count);
                        string sr = Encoding.GetEncoding(866).GetString(Encoding.Default.GetBytes(builder.ToString()));
                        errorWorker.ReportProgress(0, sr);   
                    }                    
                } while (count > 0);
